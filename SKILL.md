...
10. 指针式宪法协同（根规则 1~3 句挂链接）
根目录 `AGENTS.md` 作为极高密度的系统宪法，规则保持在 1 到 3 句话内自包含陈述，系统级中枢决策在句末以超链接挂载 Note 指针；模块级局部决策、算法流转与 Bugfix 严禁随意写入根目录规则，防止根规则膨胀与注意力稀释。详见 [when-to-write.md](references/when-to-write.md)。
